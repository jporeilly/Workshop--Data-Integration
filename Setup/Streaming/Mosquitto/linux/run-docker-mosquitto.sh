#!/bin/bash
# Bash Script to Run Cedalo Mosquitto Docker Container
...
show_help() {
    cat << EOF
${GREEN}MQTT Mosquitto Docker Deployment Script${NC}
...
${CYAN}ACTIONS:${NC}
    start               Start the Mosquitto container (default)
...
${CYAN}REQUIREMENTS:${NC}
    ...
    - Proper directory structure (run copy-mosquitto.sh first)
...
check_directory_structure() {
    ...
    if [[ ${#missing_dirs[@]} -gt 0 ]]; then
        ...
        log_info "Run the setup script first: sudo ./copy-mosquitto.sh"
        exit 1
    fi
...
check_docker_compose_file() {
    ...
    if [[ ! -f "$compose_file" ]]; then
        ...
        log_info "Please ensure docker-compose.yml is in the mosquitto directory"
        return 1
    fi
...
start_mosquitto_container() {
    ...
    log_info "Starting Cedalo Mosquitto container..."

    # Pull the latest image if not skipped
    if [[ "$SKIP_PULL" == false ]]; then
        log_info "Pulling latest Cedalo Mosquitto image..."
    ...
    # Start the container
    log_info "Starting Mosquitto container..."
    ...
    if [[ "$DRY_RUN" == false ]]; then
        ...
        local container_id=$(docker ps -q --filter "name=mosquitto-broker" 2>/dev/null || true)
        if [[ -n "$container_id" ]]; then
            log_success "Mosquitto broker is running (Container ID: $container_id)"
        else
...
show_status() {
    ...
    if [[ -n "$container_id" ]]; then
        echo ""
        log_success "Mosquitto broker is running"
        ...
        echo ""
        log_warn "Mosquitto broker is not running"
    fi
}
...
test_mqtt_connectivity() {
    ...
    local ports=(1883 9001 8088)
    local port_names=("MQTT" "WebSocket" "Management Center")
...
show_connection_info() {
    ...
    echo ""
    echo -e "${WHITE}Management Center UI:${NC}"
    echo -e "  URL: ${CYAN}http://localhost:8088${NC}"
    echo -e "  Username: cedalo"
    echo -e "  Password: password"
    echo ""
    echo -e "${WHITE}Configuration:${NC}"
    echo -e "  Config: $BASE_DIR/config/mosquitto.conf"
...
show_management_commands() {
    ...
    echo -e "${GRAY}  docker stats mosquitto-broker${NC}"
    echo -e "${GRAY}  docker exec -it mosquitto-broker sh${NC}"
}

# Main execution
main() {
    echo -e "${GREEN}=== Mosquitto Docker Deployment v$SCRIPT_VERSION ===${NC}"
    ...
    case "$ACTION" in
        start)
            ...
            start_mosquitto_container "$compose_version"
            ...
            if [[ "$DRY_RUN" == false ]]; then
                ...
                log_success "Management Center at http://localhost:8088"
