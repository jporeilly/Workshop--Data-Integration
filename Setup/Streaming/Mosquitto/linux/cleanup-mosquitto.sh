#!/bin/bash
# Bash Script to Clean Up/Uninstall Cedalo Mosquitto
...
show_help() {
    cat << EOF
${GREEN}MQTT Mosquitto Cleanup/Uninstall Script${NC}
...
${CYAN}DESCRIPTION:${NC}
    This script completely removes the Mosquitto MQTT broker installation:
...
confirm_action() {
    ...
    echo -e "  • Docker containers (mosquitto-broker)"
    ...
    if [[ "$REMOVE_IMAGES" == true ]]; then
        echo -e "  • ${RED}Docker images (cedalo/management-center)${NC}"
    fi
...
stop_and_remove_containers() {
    ...
    local container_id=$(docker ps -aq --filter "name=mosquitto-broker" 2>/dev/null || true)
    if [[ -n "$container_id" ]]; then
        ...
        log_verbose "No mosquitto-broker container found"
    fi
}
...
remove_images() {
    ...
    local images=$(docker images -q cedalo/management-center 2>/dev/null || true)
    if [[ -n "$images" ]]; then
        if [[ "$DRY_RUN" == true ]]; then
            log_dryrun "Would remove images: cedalo/management-center"
        else
            docker rmi $(docker images -q cedalo/management-center) 2>/dev/null && log_success "Removed Docker images" || log_warn "Could not remove some images (may be in use)"
        fi
    else
        log_verbose "No cedalo/management-center images found"
    fi
}
...
remove_directories() {
    ...
    # Remove Docker files
    for file in docker-compose.yml run-docker-mosquitto.sh README.md; do
...
show_summary() {
    ...
    echo -e "  ${CYAN}✓${NC} Docker containers (mosquitto-broker)"
    ...
    if [[ "$REMOVE_IMAGES" == true ]]; then
        echo -e "  ${CYAN}✓${NC} Docker images (cedalo/management-center)"
    fi
...
# Main execution
main() {
    echo -e "${GREEN}=== Mosquitto Cleanup Script v$SCRIPT_VERSION ===${NC}"
