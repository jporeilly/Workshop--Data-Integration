...
function Test-Dependencies {
    ...
    if ($missingDeps.Count -gt 0) {
        ...
        Write-Host "  # MinIO Client:" -ForegroundColor Gray
        Write-Host "  # Download from: https://dl.min.io/client/mc/release/windows-amd64/mc.exe" -ForegroundColor Gray
        Write-Host "  # Place in PATH (e.g., C:\Windows\System32 or C:\Program Files\MinIO)" -ForegroundColor Gray
        return $false
    }
...
function Test-MinIOConnectivity {
    ...
    try {
        ...
        Write-Host "  .\run-docker-minio.ps1" -ForegroundColor Gray
        return $false
    }
}
...
function New-MinioBuckets {
    ...
    foreach ($bucket in $buckets) {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  [EXISTS] Bucket: $bucket" -ForegroundColor Gray
        }
        else {
            # Create bucket
...
function Show-Summary {
    ...
    Write-Host "  1. Configure VFS S3 connection in PDI:" -ForegroundColor White
    Write-Host "     Endpoint: $MinIOEndpoint" -ForegroundColor Gray
    Write-Host "     Access Key: $MinIOUser" -ForegroundColor Gray
    Write-Host "     Secret Key: $MinIOPassword" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  2. Create transformations to read from buckets:" -ForegroundColor White
    Write-Host "     s3a://${BUCKET_RAW}/csv/customers.csv" -ForegroundColor Gray
    Write-Host "     s3a://${BUCKET_RAW}/json/api_response.json" -ForegroundColor Gray
    Write-Host "     s3a://${BUCKET_RAW}/xml/inventory.xml" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  3. Explore data using MinIO Client:" -ForegroundColor White
    Write-Host "     mc ls ${MinIOAlias}/${BUCKET_RAW} --recursive" -ForegroundColor Gray
    Write-Host ""
}
